...
export default class UrlParser {
    ...
    return new Promise((resolve) => {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        resolve({
          ...
          testExtension:
            (fragmentParts.length > 1 && this._parseTestExtension(fragmentParts[1])) || undefined,
        });
      });
    });
  }
  ...
  public setUrl(urlComponents: IUrlComponents): Promise<void> {
    ...
    Object.keys(urlComponents.query).forEach((key) =>
      url.searchParams.set(key, urlComponents.query[key]),
    );
    url.hash = `${urlComponents.fragment}${
      urlComponents.testExtension
        ? `?${this._stringifyTestExtension(urlComponents.testExtension, urlComponents.sideloadUrl)}`
        : ""
    }`;
  ...
  private _parseTestExtension(fragmentQuery: string): string | null {
    ...
    const match = fragmentQuery.match(
      /^testextensions={(?:"|%22)(.*?)(?:"|%22):(?:"|%22)(.*?)(?:"|%22)}$/i,
    );
    return match && match[1];
  }

  private _stringifyTestExtension(testExtension: string, sideloadUrl?: string) {
    return `testExtensions={"${testExtension}":"${
      sideloadUrl != null ? sideloadUrl : "https://localhost:44300/"
    }"}`;
  }
}
